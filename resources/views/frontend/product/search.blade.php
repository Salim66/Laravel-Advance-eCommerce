          ...
          <div class="row align-items-center">
             <div class="col-md-7">
                <div class="header-content">
                   <h1>
                       @if(session()->get('language') == 'arabic') البحث عن المنتجات @else Search Products @endif 
                    </h1>
                   <nav aria-label="breadcrumb">
                      <ol class="breadcrumb">
                         <li class="breadcrumb-item"><a href="{{ url('/') }}">@if(session()->get('language') == 'arabic') مسكن @else Home @endif</a></li>
                         <li class="breadcrumb-item active" aria-current="page">@if(session()->get('language') == 'arabic') البحث عن المنتجات @else Search Products @endif </li>
                      </ol>
                   </nav>
                </div>
             </div>
    ...
    <div class="container">
       <div class="product-header">
          <div class="result-block product-header-item m-auto mb-5">
             <h2>
                @if(session()->get('language') == 'arabic') إجمالي البحث @else Total Search @endif 
             </h2>
          </div>
       </div>

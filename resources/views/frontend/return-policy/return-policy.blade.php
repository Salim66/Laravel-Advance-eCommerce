          ...
          <div class="row align-items-center">
             <div class="col-md-7">
                <div class="header-content">
                   <h1>@if(session()->get('language') == 'arabic') سياسة العائدات @else Return Policy @endif</h1>
                   <nav aria-label="breadcrumb">
                      <ol class="breadcrumb">
                         <li class="breadcrumb-item"><a href="{{ url('/') }}">@if(session()->get('language') == 'arabic') مسكن @else Home @endif </a></li>
                         <li class="breadcrumb-item active" aria-current="page">@if(session()->get('language') == 'arabic') سياسة العائدات @else Return Policy @endif</li>
                      </ol>
                   </nav>
                </div>
             </div>
...
<section class="privacy-policy-section p-tb-100">
    <div class="container">
       <div class="forum-details">
        @if(session()->get('language') == 'arabic') {!! $return->return_policy_descp_ar !!}  @else {!! $return->return_policy_descp_en !!}  @endif
       </div>
    </div>
</section>
